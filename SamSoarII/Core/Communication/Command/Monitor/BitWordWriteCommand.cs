...
{
    class BitWordWriteCommand : ICommunicationCommand
    {
        ...
        public BitWordWriteCommand(byte[] data, ValueStore vstore)
        {
            data1 = data;
        ...
        {
            addrTypeNum = 0x01;
            addrType1 = (byte)CommandHelper.GetBitWordAddrType(vstore.Base, (uint)(vstore.Offset));
            length1 = 1;
            wordlength = (byte)(vstore.Flag);
            byte[] startaddr = ValueConverter.GetBytes((ushort)(vstore.Offset >> 4));
        ...
        private void GenerateCommand()
        {
            ...
            for (int i = 0; i < commandCache.Length; i++)
            {
                ...
                else if (i == 7) commandCache[i] = wordlength;
